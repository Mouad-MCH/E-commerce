...
export const clerkWebhookHandler = async (req: Request, res: Response) => {
    try {
        ...
        // throws if signature is wrong or body was tamperd with; only then we trust evt.
        ...
        if(evt.type === "user.created" || evt.type === "user.updated") {
            ...
            const email =
                u.email_addresses?.find((e) => e.id === u.
                primary_email_address_id)?.email_address ??
                u.email_addresses?.[0]?.email_address;
            
            const displayName = 
                [u.first_name, u.last_name].filter(Boolean).join(" ") || u.
                username || null;
